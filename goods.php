...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
		...
		甜心新增
		*/
		$rank_price=get_user_rank_prices($goods_id, $shop_price);
		$user_rankid=$_SESSION['user_rank'];
		$new=array();
		foreach($rank_price  as $k=>$v){
			if($k==$user_rankid){
				$new["rank_name"]=$v['rank_name'];
				$new["price"]=$v['price'];
			}
		}
		//如果用户未登陆
